...
size_t MetaGraphDX::addShapeGraph(std::unique_ptr<ShapeGraphDX> &&shapeGraph) {
...
size_t MetaGraphDX::addShapeGraph(std::unique_ptr<ShapeGraph> &&shapeGraph) {
    return addShapeGraph(std::make_unique<ShapeGraphDX>(std::move(shapeGraph)));
}
...
size_t MetaGraphDX::addShapeGraph(const std::string &name, int type) {
    auto mapref =
        addShapeGraph(std::make_unique<ShapeGraphDX>(std::make_unique<ShapeGraph>(name, type)));
    ...
    try {
        ...
        if (sourcemap->getInternalMap().getShapeCount() != 0) {
            ...
            for (; group != m_drawingFiles.end(); ++group) {
                if (group->first.getName() == "Converted Maps") {
                    break;
                }
            }
            if (group == m_drawingFiles.end()) {
                m_drawingFiles.emplace_back("Converted Maps", std::vector<ShapeMapDX>());
                group = std::prev(m_drawingFiles.end());
            }
            group->second.emplace_back(layer_name, ShapeMap::DRAWINGMAP);
            group->second.back().copy(*sourcemap, ShapeMap::COPY_GEOMETRY);
            //
            // dummy set still required:
            group->second.back().invalidateDisplayedAttribute();
            group->second.back().setDisplayedAttribute(-1);
            //
            // three levels of merge region:
            if (group->second.size() == 1) {
                group->first.m_region = group->second.back().getRegion();
            } else {
                group->first.m_region =
                    runion(group->first.m_region, group->second.back().getRegion());
            }
            if (m_drawingFiles.size() == 1) {
                m_metaGraph.region = group->first.m_region;
            } else {
                m_metaGraph.region = runion(m_metaGraph.region, group->first.m_region);
            }
            //
            converted = true;
        }
                               ...
                               bool) // options copied to keep thread safe
{
    ...
    try {
        ...
        map.setDisplayedAttribute(AxialIntegration::getFormattedColumnIdx(
...
bool MetaGraphDX::analyseSegmentsTulip(Communicator *communicator,
                                       Options options) // <- options copied to keep thread safe
{
    ...
    try {
        ...
        map.setDisplayedAttribute(-2); // <- override if it's already showing
        if (options.choice) {
            map.setDisplayedAttribute(SegmentTulip::getFormattedColumnIdx(
...
int MetaGraphDX::loadLineData(Communicator *communicator, int load_type) {
    ...
    m_drawingFiles.emplace_back(communicator->GetMBInfileName(), std::vector<ShapeMapDX>());
    ...
    for (auto &shapeMap : maps) {
        newDrawingMap.second.push_back(std::make_unique<ShapeMap>(std::move(shapeMap)));
        auto &shpmdx = newDrawingMap.second.back();
    ...
    if (m_drawingFiles.size() == 1) {
        m_metaGraph.region = m_drawingFiles.back().first.m_region;
    } else {
        m_metaGraph.region = runion(m_metaGraph.region, m_drawingFiles.back().first.m_region);
    }
...
ShapeMapDX &MetaGraphDX::createNewShapeMap(depthmapX::ImportType mapType, std::string name) {
    ...
    m_drawingFiles.back().second.emplace_back(name, ShapeMap::DRAWINGMAP);
    return m_drawingFiles.back().second.back();
}
...
void MetaGraphDX::deleteShapeMap(depthmapX::ImportType mapType, ShapeMapDX &shapeMap) {
    ...
    case depthmapX::ImportType::DRAWINGMAP: {
        ...
        for (; pixelGroup != m_drawingFiles.begin(); ++pixelGroup) {
            auto mapToRemove = pixelGroup->second.end();
            auto pixel = pixelGroup->second.begin();
            for (; pixel != pixelGroup->second.end(); ++pixel) {
                if (&(*pixel) == &shapeMap) {
                    mapToRemove = pixel;
                    break;
                }
            }
            if (mapToRemove != pixelGroup->second.end()) {
                pixelGroup->second.erase(mapToRemove);
                if (pixelGroup->second.size() == 0) {
...
void MetaGraphDX::updateParentRegions(ShapeMap &shapeMap) {
    if (m_drawingFiles.back().first.m_region.atZero()) {
        m_drawingFiles.back().first.m_region = shapeMap.getRegion();
    } else {
        m_drawingFiles.back().first.m_region =
            runion(m_drawingFiles.back().first.m_region, shapeMap.getRegion());
    }
    ...
    try {
        ...
        m_metaGraph = mgd.metaGraph;
        {
            ...
            for (auto &&mapGroup : mgd.drawingFiles) {
                m_drawingFiles.emplace_back(ShapeMapGroupDataDX(std::move(mapGroup.first)),
                                            std::vector<ShapeMapDX>());
                auto ddIt = gddIt->begin();
                for (auto &&map : mapGroup.second) {
                    m_drawingFiles.back().second.emplace_back(
                        std::make_unique<ShapeMap>(std::move(map)));
                    auto &newMapDX = m_drawingFiles.back().second.back();
        ...
        }
        {
            ...
            for (auto &&map : mgd.shapeGraphs) {
                m_shapeGraphs.push_back(std::make_unique<ShapeGraphDX>(std::move(map)));
...
MetaGraphReadWrite::ReadStatus MetaGraphDX::write(const std::string &filename, int version,
    ...
    for (auto &mapGroupDX : m_drawingFiles) {
        drawingFiles.push_back(std::make_pair(mapGroupDX.first.getInternalData(),
        ...
        for (auto &mapDX : mapGroupDX.second) {
...
void MetaGraphDX::makeViewportShapes(const QtRegion &viewport) const {
    ...
    for (auto iter = m_drawingFiles.rbegin(); iter != m_drawingFiles.rend(); iter++) {
        if (isShown(*iter)) {
            ...
            iter->first.invalidateCurrentLayer();
            for (size_t i = iter->second.size() - 1; static_cast<int>(i) != -1; i--) {
                if (iter->second[i].isShown()) {
                    if (!iter->second[i].isValid()) {
                        continue;
                    }
                    iter->first.setCurrentLayer(i);
                    iter->second[i].makeViewportShapes(
...
bool MetaGraphDX::findNextShape(const std::pair<ShapeMapGroupDataDX, std::vector<ShapeMapDX>> &spf,
                                bool &nextlayer) const {
    if (!spf.first.hasCurrentLayer())
        return false;
    while (spf.second[spf.first.getCurrentLayer()].valid() &&
           !spf.second[spf.first.getCurrentLayer()].findNextShape(nextlayer)) {
        spf.first.setCurrentLayer(spf.first.getCurrentLayer() + 1);
        while (spf.first.getCurrentLayer() < (int)spf.second.size() &&
               !spf.second[spf.first.getCurrentLayer()].isShown())
            spf.first.setCurrentLayer(spf.first.getCurrentLayer() + 1);
        if (spf.first.getCurrentLayer() == static_cast<int>(spf.second.size())) {
            spf.first.invalidateCurrentLayer();
            return false;
        }
    }
    return true;
...
bool MetaGraphDX::findNextShape(bool &nextlayer) const {
    if (m_current_layer == -1)
        return false;
    while (!findNextShape(m_drawingFiles[m_current_layer], nextlayer)) {
        while (++m_current_layer < (int)m_drawingFiles.size() &&
               !isShown(m_drawingFiles[m_current_layer]))
            ;
