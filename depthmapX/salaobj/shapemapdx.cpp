...
void ShapeMapDX::setDisplayedAttribute(const std::string &col) {
    setDisplayedAttribute(getInternalMap().getAttributeTable().getColumnIndex(col));
}
...
float ShapeMapDX::getDisplayedAverage() {
    return (getInternalMap().getDisplayedAverage(static_cast<size_t>(m_displayed_attribute)));
}
...
bool ShapeMapDX::write(std::ostream &stream) {
    ...
    int sortedDisplayedAttribute =
        getInternalMap().getAttributeTable().getColumnSortedIndex(m_displayed_attribute);
...
bool ShapeMapDX::findNextShape(bool &nextlayer) const {
    ...
    while ((++m_current_shape < (int)getInternalMap().getAllShapes().size()) &&
           m_display_shapes[m_current_shape] == static_cast<size_t>(-1))
        ;
...
const SalaShape &ShapeMapDX::getNextShape() const {
    int x = m_display_shapes[m_current_shape]; // x has display order in it
    m_display_shapes[m_current_shape] = -1;    // you've drawn it
...
void ShapeMapDX::makeViewportShapes(const QtRegion &viewport) const {
    ...
    if (m_display_shapes.empty() || m_newshape) {
        m_display_shapes.assign(shapes.size(), -1);
        m_newshape = false;
    }
...
void ShapeMapDX::makeShapeConnections() {
    ...
    int conn_col = getInternalMap().getAttributeTable().getColumnIndex("Connectivity");
...
bool ShapeMapDX::getShapeSelected() const {
    return m_selectionSet.find(m_display_shapes[static_cast<size_t>(m_current_shape)]) !=
           m_selectionSet.end();
}
...
Line ShapeMapDX::getNextLinkLine() const {
    ...
    if (m_curlinkline < (int)getInternalMap().getLinks().size()) {
        return Line(depthmapX::getMapAtIndex(getInternalMap().getAllShapes(),
                                             getInternalMap().getLinks()[m_curlinkline].a)
                        ->second.getCentroid(),
                    depthmapX::getMapAtIndex(getInternalMap().getAllShapes(),
                                             getInternalMap().getLinks()[m_curlinkline].b)
                        ->second.getCentroid());
    }
    return Line();
}
...
Point2f ShapeMapDX::getNextUnlinkPoint() const {
    ...
    if (m_curunlinkpoint < (int)getInternalMap().getUnlinks().size()) {
        return intersection_point(
            depthmapX::getMapAtIndex(getInternalMap().getAllShapes(),
                                     getInternalMap().getUnlinks()[m_curunlinkpoint].a)
                ->second.getLine(),
            depthmapX::getMapAtIndex(getInternalMap().getAllShapes(),
                                     getInternalMap().getUnlinks()[m_curunlinkpoint].b)
                ->second.getLine(),
            TOLERANCE_A);
    }
    return Point2f();
