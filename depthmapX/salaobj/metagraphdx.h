...
class MetaGraphDX {
  ...
  public:
    ...
    bool isShown() const {
        for (auto &drawingFile : m_drawingFiles)
            for (auto &map : drawingFile.second)
                if (map.isShown())
                    return true;
        return false;
    }
    ...
    bool isShown(const std::pair<ShapeMapGroupDataDX, std::vector<ShapeMapDX>> &spf) const {
        for (auto &pixel : spf.second)
            if (pixel.isShown())
                return true;
        return false;
    }
    ...
    bool findNextShape(const std::pair<ShapeMapGroupDataDX, std::vector<ShapeMapDX>> &spf,
    ...
    const SalaShape &getNextShape() const {
        ...
        return currentDrawingFile.second[currentDrawingFile.first.getCurrentLayer()].getNextShape();
    }
  ...
  public:
    ...
    const std::string &getLineFileName(size_t fileIdx) const {
        return m_drawingFiles[fileIdx].first.getName();
    }
    size_t getLineLayerCount(size_t fileIdx) const { return m_drawingFiles[fileIdx].second.size(); }
