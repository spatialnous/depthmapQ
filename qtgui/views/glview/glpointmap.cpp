...
void GLPixelMap::loadGLObjects(PointMapDM &pointMap) {
    ...
    if (m_showLinks) {
        const std::vector<SimpleLine> &mergedPixelLines =
            sala::getMergedPixelsAsLines(pointMap.getInternalMap());
...
void GLPixelMap::loadGLObjectsRequiringGLContext(const PointMapDM &currentPointMap) {
