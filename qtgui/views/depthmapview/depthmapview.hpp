...
#include "qtgui/views/mapview.hpp"
...
class QDepthmapView : public MapView {
  ...
  private:
    ...
    bool DrawAxial(QPainter *pDC, QGraphDoc *pDoc, int spacer, unsigned long ticks,
                   bool screendraw);
    bool DrawShapes(QPainter *pDC, ShapeMapDM &map, bool muted, int spacer, unsigned long ticks,
                    bool screendraw);
    ...
    void OutputEPSMap(std::ofstream &stream, ShapeMapDM &map, Region4f &logicalviewport,
    ...
    void OutputSVGMap(std::ofstream &stream, ShapeMapDM &map, Region4f &logicalviewport, int h);
