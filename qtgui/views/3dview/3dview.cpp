...
void Q3DView::ReloadLineData() {
    ...
    if (pDoc->m_meta_graph && pDoc->m_meta_graph->getState() & MetaGraphDM::DX_LINEDATA) {
...
void Q3DView::OnToolsAgentLoadProgram() {
    ...
    if (!filename.empty()) {
        ...
        if (!m_agent_program.open(filename)) {
            QMessageBox::warning(this, tr("Notice"), tr("Unable to understand agent program"),
                                 QMessageBox::Ok, QMessageBox::Ok);
        }
    } else {
        QMessageBox::warning(this, tr("Notice"), tr("No file selected"), QMessageBox::Ok,
                             QMessageBox::Ok);
    }
}
