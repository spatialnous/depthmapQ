...
#include "qtgui/views/3dview/3dview.hpp"
#include "qtgui/views/depthmapview/depthmapview.hpp"
#include "qtgui/views/plotview/plotview.hpp"
#include "qtgui/views/tableview/tableview.hpp"
...
void MainWindow::loadFile(QString fileName) {
            ...
            file)) // quick fix for weird chars (russian filename bug report)
    {
        ...
        child->close();
        QMessageBox::warning(this, "Failed to load", QString("Failed to load file ") + fileName,
...
void MainWindow::OnFileOpen() {
    ...
    QString fileName =
        QFileDialog::getOpenFileName(0, tr("Open"), "", template_string, &selectedFilter, options);
    if (!fileName.isEmpty()) {
        loadFile(fileName);
    }
}
...
void MainWindow::OnViewCentreView() {
    activeMapDoc()->SetRedrawFlag(QGraphDoc::VIEW_MAP, QGraphDoc::REDRAW_TOTAL,
...
void MainWindow::OnHelpBugs() {
    bool foo = QDesktopServices::openUrl(QUrl("https://github.com/spatialnous/depthmapQ/issues"));
}
...
int MainWindow::OnFocusGraph(QGraphDoc *pDoc, int lParam) {
    ...
    if (lParam == QGraphDoc::CONTROLS_DESTROYALL && pDoc == m_treeDoc) { // Lost graph
        ...
        m_backgraph = NULL;
        MakeTree();
    } else if (lParam == QGraphDoc::CONTROLS_LOADGRAPH &&
        ...
        MakeDrawingTree();
               ...
               pDoc == m_treeDoc) { // Force reload of graph tree if match current
                                    // window
        m_topgraph = NULL;
        m_backgraph = NULL;
        m_attrWindow->clear();
...
void MainWindow::MakeTree() {
    ...
    auto *graph = m_treeDoc->m_meta_graph;
    if (!graph)
        return;
...
void MainWindow::OnSelchangingList() {
    ...
    if (row > -1 && m_treeDoc) {
        ...
        m_treeDoc->SetRedrawFlag(QGraphDoc::VIEW_ALL, QGraphDoc::REDRAW_GRAPH,
                                 QGraphDoc::NEW_FOCUS);
        SetAttributeChecks();
        ...
        if (entry.m_cat != -1) {
            if (entry.m_subcat == -1 && m_indexWidget->isMapColumn(col)) {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    if (graph->getViewClass() & MetaGraphDM::DX_VIEWAXIAL) {
                        if (graph->getDisplayedShapeGraphRef() == entry.m_cat) {
                            graph->setViewClass(MetaGraphDM::DX_SHOWHIDEAXIAL);
                        } else {
                            graph->setDisplayedShapeGraphRef(entry.m_cat);
                        }
                    } else {
                        graph->setDisplayedShapeGraphRef(entry.m_cat);
                        graph->setViewClass(MetaGraphDM::DX_SHOWAXIALTOP);
                    }
                    remenu = true;
                    break;
                case 2:
                    if (graph->getViewClass() & MetaGraphDM::DX_VIEWDATA) {
                        if (graph->getDisplayedDataMapRef() == entry.m_cat) {
                            graph->setViewClass(MetaGraphDM::DX_SHOWHIDESHAPE);
                        } else {
                            graph->setDisplayedDataMapRef(entry.m_cat);
                        }
                    } else {
                        graph->setDisplayedDataMapRef(entry.m_cat);
                        graph->setViewClass(MetaGraphDM::DX_SHOWSHAPETOP);
                    }
                    remenu = true;
                    break;
                case 4:
                ...
                if (entry.m_type == 2) {
                    graph->getDataMaps()[entry.m_cat].setEditable(
                        m_indexWidget->isItemSetEditable(hItem));
                    update = true;
                }
                if (update) {
                    // Depending on if the map is displayed you may have to redraw -- I'm
                    // just going to redraw *anyway* (it may be worth switching it to
                    // topmost when they do click here)
                    OnFocusGraph(m_treeDoc, QGraphDoc::CONTROLS_CHANGEATTRIBUTE);
                }
            } else {
                ...
                if (entry.m_type == 1) {
                    ...
                    update = true;
                    graph->getDataMaps()[entry.m_cat].getInternalMap().setLayerVisible(
                        entry.m_subcat, m_indexWidget->isItemSetVisible(hItem));
                }
                if (update) {
                    m_treeDoc->SetRedrawFlag(QGraphDoc::VIEW_ALL, QGraphDoc::REDRAW_GRAPH,
        ...
        if (entry.m_cat != -1) {
            if (entry.m_subcat == -1) {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    if (viewclass & MetaGraphDM::DX_VIEWAXIAL &&
                        graph->getDisplayedShapeGraphRef() == entry.m_cat) {
                        checkstyle = 5;
                        m_topgraph = key;
                    } else if (viewclass & MetaGraphDM::DX_VIEWBACKAXIAL &&
                               graph->getDisplayedShapeGraphRef() == entry.m_cat) {
                        checkstyle = 6;
                        m_backgraph = key;
                    }
                ...
                case 2:
                    if (viewclass & MetaGraphDM::DX_VIEWDATA &&
                        ...
                        checkstyle = 5;
                        m_topgraph = key;
                    } else if (viewclass & MetaGraphDM::DX_VIEWBACKDATA &&
                               graph->getDisplayedDataMapRef() == entry.m_cat) {
                        checkstyle = 6;
                        m_backgraph = key;
                    }
                ...
                case 1: {
                    ...
                    if (type == ShapeMap::SEGMENTMAP || type == ShapeMap::ALLLINEMAP) {
                        ...
                        editable = graph->getShapeGraphs()[entry.m_cat].isEditable()
                                       ? MetaGraphDM::DX_EDITABLE_ON
                                       : MetaGraphDM::DX_EDITABLE_OFF;
                    }
                } break;
                case 2:
                ...
                switch (editable) {
                case MetaGraphDM::DX_NOT_EDITABLE:
                    m_indexWidget->setItemReadOnly(key);
                    break;
                case MetaGraphDM::DX_EDITABLE_OFF:
                    m_indexWidget->setItemEditability(key, Qt::Unchecked);
                    break;
                case MetaGraphDM::DX_EDITABLE_ON:
...
void MainWindow::RedoPlotViewMenu(QGraphDoc *pDoc) {
    ...
    int view_class =
        ...
        (MetaGraphDM::DX_VIEWVGA | MetaGraphDM::DX_VIEWAXIAL | MetaGraphDM::DX_VIEWDATA);
    int curr_j = 0;

    {
        ...
        if (lock.try_lock()) {
            ...
            } else if (view_class == MetaGraphDM::DX_VIEWAXIAL) {
...
void MainWindow::OnSelchangeViewSelector_X(int) {
    ...
    QGraphDoc *gd = activeMapDoc();
    ((QPlotView *)gd->m_view[QGraphDoc::VIEW_SCATTER])
        ->SetAxis(0, /*m_view_selection*/ i - 1, true);
    ((QPlotView *)gd->m_view[QGraphDoc::VIEW_SCATTER])->curr_x = i;

    // note: this is only attached to a scatter view, and changing the attribute
    // only affects the scatter view, so only send draw to the map:
...
void MainWindow::updateViewMenu() {
    ...
    if (m_p->m_meta_graph->getShowGrid())
...
void MainWindow::updateVisibilitySubMenu() {
    ...
    if (m_p->m_meta_graph->getState() & MetaGraphDM::DX_LINEDATA ||
...
void MainWindow::updateStepDepthSubMenu() {
    ...
    if ((m_p->m_meta_graph->viewingProcessedPoints() ||
         (m_p->m_meta_graph->viewingProcessedLines() &&
          m_p->m_meta_graph->getDisplayedShapeGraph().getInternalMap().isSegmentMap())) &&
...
void MainWindow::updateAxialSubMenu() {
    ...
    if (state & MetaGraphDM::DX_SHAPEGRAPHS && !m_p->m_communicator &&
        !m_p->m_meta_graph->getDisplayedShapeGraph().isSegmentMap())
...
void MainWindow::updateMapMenu() {
    ...
    if (!m_p->m_communicator &&
        (m_p->m_meta_graph->getState() & MetaGraphDM::DX_LINEDATA) == MetaGraphDM::DX_LINEDATA)
...
void MainWindow::updateToolbar() {
    ...
    if (m_p) {
        ...
        if ((type == ShapeMap::DATAMAP && m_p->m_meta_graph->getDisplayedDataMap().isEditable()) ||
            ((type == ShapeMap::AXIALMAP || type == ShapeMap::CONVEXMAP ||
              type == ShapeMap::PESHMAP) &&
        ...
        if (!(~type & MetaGraphDM::DX_LINEDATA))
        ...
        if ((((m_p->m_meta_graph->getViewClass() & MetaGraphDM::DX_VIEWVGA) &&
            ...
            if (tmpView) {
                if (m_selected_mapbar_item == ID_MAPBAR_ITEM_JOIN ||
                    m_selected_mapbar_item == ID_MAPBAR_ITEM_UNJOIN) {
                    tmpView->OnEditSelect();
                    SelectButton->setChecked(true);
                }
...
void MainWindow::createActions() {
    ...
    saveAsAct->setStatusTip(
        tr("Save the active graph workspace with a new name\nSave Workspace As"));
    ...
    exportScreenAct->setStatusTip(
        tr("Export the screen as an Encapsulated Postscript file\nExport Screen"));
    ...
    // to Selection")); connect(zoomToSelectionAct, SIGNAL(triggered()), this,
    // SLOT(OnViewZoomsel()));
    ...
    connect(exportAxialConnectionsPairAct, SIGNAL(triggered()), this,
            SLOT(OnAxialConnectionsExportAsPairCSV()));
    ...
    connect(exportAxialConnectionsDotAct, SIGNAL(triggered()), this,
            SLOT(OnAxialConnectionsExportAsDot()));

    exportSegmentConnectionsPairAct = new QAction(tr("&Segment Connections as CSV..."), this);
    exportSegmentConnectionsPairAct->setStatusTip(
        tr("Export a list of line-line intersections and weights"));
    ...
    connect(generateIsovistsAct, SIGNAL(triggered()), this,
            SLOT(OnToolsGenerateIsovistsFromFile()));
    ...
    convertDataMapLinesAct->setStatusTip(
        tr("Convert displayed data map lines to merge points for current "
           "visibility graph"));
    ...
    runGraphAnaysisAct->setStatusTip(
        tr("Analyse currently displayed axial line map\nAxial analysis"));
    ...
    convertDataMapPointsAct->setStatusTip(
        tr("Convert displayed data map points to unlinks for current axial map"));
    ...
    attributeSummaryAct->setStatusTip(
    ...
    //	connect(arrangeIconsAct, SIGNAL(triggered()), mdiArea,
    // SLOT(arrangeSubWindows()));
    ...
    aboutDepthMapAct = new QAction(tr("About %1...").arg(APP_NAME.c_str()), this);
    aboutDepthMapAct->setStatusTip(
        tr("Display program information, version number and copyright\nAbout"));
    ...
    updateColumAct->setStatusTip(
        tr("Replace column contents using a SalaScript command\nUpdate Column"));
    ...
    toggleOrg->setStatusTip(
        tr("Toggle graph intersect at origin of X, Y values\nToggle origin on/off"));
    ...
    SelectPenButton->setStatusTip(tr("Fill grid spaces individually (or click on "
                                     "a filled space to clear)\nPencil"));
    ...
    {
        toolsImportTracesAct =
            new QAction(QIcon(":/images/win/b-4-1.png"), tr("Import Traces"), this);
    ...
    }
    {
        ...
        toolsAgentsPauseAct =
            new QAction(QIcon(":/images/win/b-4-4.png"), tr("Agents Pause"), this);
...
void MainWindow::createMenus() {
    ...
    connect(segmentStepDepthSubMenu, SIGNAL(aboutToShow()), this,
            SLOT(updateSegmentStepDepthSubMenu()));
...
void MainWindow::createToolBars() {
    ...
    connect(x_coord, SIGNAL(currentIndexChanged(int)), this, SLOT(OnSelchangeViewSelector_X(int)));
    connect(y_coord, SIGNAL(currentIndexChanged(int)), this, SLOT(OnSelchangeViewSelector_Y(int)));
